...
impl Header {
    ...
    fn verify_sub_chain(&self, chain: &[Header]) -> bool {
        let mut current_valid_height = self.height;
        if chain.len() == 1 {
            if self.height != chain[0].height + 1 { return false; }
            if hash(self) != chain[0].parent { return false; }
        }

        for i in 1..chain.len() {
            current_valid_height += 1;
            let parent = &chain[i - 1];
            let child = &chain[i];
            if parent.height != current_valid_height { return false; }

            if hash(parent) != child.parent {
                return false;
            }
        }
        return true;
...
fn build_valid_chain_length_5() -> Vec<Header> {
    ...
    return vec![block1, block2, block3, block4, block5];
}
...
fn build_an_invalid_chain() -> Vec<Header> {
    ...
    return vec![block1, block2, block3, block4];
}
